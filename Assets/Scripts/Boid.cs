...
public class Boid : MonoBehaviour
{
    ...
    {
        if (Vector3.Distance(transform.localPosition, transform.parent.position) > 20f)
        {
            //thisRigidbody.AddForce((Vector3.zero - transform.localPosition) * 0.01f);
            thisRigidbody.velocity += (transform.parent.position - transform.localPosition) * 0.1f * Time.deltaTime;
        }
    }
